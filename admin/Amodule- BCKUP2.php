...
if (isset($_GET['retake'])) {
    if ($_GET['retake'] === 'enabled') {
        echo '<div class="bg-green-100 text-green-800 px-4 py-2 rounded mb-4">Final quiz retake enabled for this module.</div>';
    } elseif ($_GET['retake'] === 'disabled') {
        echo '<div class="bg-green-100 text-green-800 px-4 py-2 rounded mb-4">Final quiz retake disabled for this module.</div>';
    } elseif ($_GET['retake'] === 'error') {
        echo '<div class="bg-red-100 text-red-800 px-4 py-2 rounded mb-4">Unable to update the retake setting. Please try again.</div>';
    }
}
?> 
<!DOCTYPE html>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            if (!empty($row['final_quiz_id'])) {
                ...
                echo '<button type="submit" class="px-3 py-1 rounded ' . $retakeButtonClasses . '">' . $retakeButtonLabel . '</button>';
                echo '</form>';
            }
...
<div id="module-parts-section" class="tab-content hidden">
        ...
        <form id="add-module-part-form" class="space-y-4" method="POST" action="database/add_module_part.php" enctype="multipart/form-data">
            <div>
                ...
                <select id="select-module" name="module_id" class="w-full px-4 py-2 border rounded-md focus:ring-blue-500 focus:border-blue-500" required>
                    ...
                    $conn_modules = new mysqli(getenv('MYSQLHOST') ?: 'tramway.proxy.rlwy.net', getenv('MYSQLUSER') ?: 'root', getenv('MYSQLPASSWORD') ?: 'niCcpkrZKKhLDhXeTbcbhIYSFJBfNibP', getenv('MYSQLDATABASE') ?: 'railway', intval(getenv('MYSQLPORT') ?: 10241));
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        $conn_parts = new mysqli(getenv('MYSQLHOST') ?: 'tramway.proxy.rlwy.net', getenv('MYSQLUSER') ?: 'root', getenv('MYSQLPASSWORD') ?: 'niCcpkrZKKhLDhXeTbcbhIYSFJBfNibP', getenv('MYSQLDATABASE') ?: 'railway', intval(getenv('MYSQLPORT') ?: 10241));
            ...
            sections.forEach((section, index) => {
                ...
                sectionDiv.innerHTML = `
                    <div class="flex justify-between items-start mb-4">
                        <div>
                            <h3 class="font-bold text-lg text-gray-800">Section ${section.id}</h3>
                            <p class="text-sm text-gray-500">ID: ${section.id}</p>
                        </div>
                        <button type="button" class="remove-section px-3 py-1 text-red-600 hover:bg-red-100 rounded-md font-medium text-sm">
                            Remove
                        </button>
                    </div>
                    ...
                    <div class="space-y-4">
                        <div>
                            <label class="block mb-2 font-semibold text-gray-700">Module Sub-Title</label>
                            <input type="text" name="sections[${section.id}][subtitle]" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500" value="${section.subtitle || ''}" required>
                        </div>
                        
                        <div>
                            <label class="block mb-2 font-semibold text-gray-700">Content</label>
                            <textarea id="section-${section.id}-content" name="sections[${section.id}][content]" rows="6" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 tinymce-editor" required>${section.content || ''}</textarea>
                        </div>
                    </div>
                `;
...
<div id="edit-quiz-modal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden items-center justify-center z-50">
  <div class="bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 overflow-y-auto max-h-[90vh]">
    <div class="border-b px-6 py-4 flex justify-between items-center">
      <h3 class="text-lg font-bold text-gray-800">Edit Final Quiz</h3>
    ...
    <form id="edit-quiz-form" class="p-6 space-y-4" method="POST" action="database/edit_final_quiz.php">
      <input type="hidden" id="edit-quiz-id" name="quiz_id">

      <div>
        <label for="edit-quiz-title" class="block mb-2 font-medium text-gray-700">Quiz Title</label>
        <input type="text" id="edit-quiz-title" name="quiz_title" class="w-full px-4 py-2 border rounded-md focus:ring-blue-500 focus:border-blue-500" required>
      </div>

      <div class="flex justify-between items-center">
        <h4 class="font-semibold text-gray-700">Quiz Questions</h4>
        <button type="button" id="add-edit-question" class="px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700">
          + Add Question
        </button>
      </div>
      ...
      <div class="flex justify-end space-x-3 pt-4">
        <button type="button" id="cancel-edit-quiz" class="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300">
          Cancel
        </button>
...
function editQuiz(quizId) {
  ...
  questionsContainer.innerHTML = '<p class="text-gray-500">Loading...</p>';
  ...
  function renderQuestion(id, text, o1, o2, o3, o4, correct, index) {
    ...
    wrapper.className = 'quiz-question bg-gray-50 p-4 border rounded-md space-y-3 relative';
    wrapper.innerHTML = `
      <button type="button" class="absolute top-2 right-2 text-red-600 hover:text-red-800 remove-question">
        ✖
      </button>
      ...
      <div>
        ...
        <input type="text" name="questions[${index}][text]" class="w-full border rounded px-3 py-2" value="${text || ''}" required>
      </div>
      <div class="grid grid-cols-2 gap-3">
        <input type="text" name="questions[${index}][option1]" class="border rounded px-3 py-2" value="${o1 || ''}" required>
        <input type="text" name="questions[${index}][option2]" class="border rounded px-3 py-2" value="${o2 || ''}" required>
        <input type="text" name="questions[${index}][option3]" class="border rounded px-3 py-2" value="${o3 || ''}" required>
        <input type="text" name="questions[${index}][option4]" class="border rounded px-3 py-2" value="${o4 || ''}" required>
      </div>
      <div>
        <label class="block text-sm text-gray-700 mb-1">Correct Answer</label>
        <select name="questions[${index}][correct]" class="border rounded px-3 py-2">
