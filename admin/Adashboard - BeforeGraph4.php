...
<div class="bg-white rounded-lg shadow-md p-6 mb-6">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <h3 class="text-lg font-semibold mb-4 text-gray-800">Time to Complete Module by Gender</h3>
        <div class="flex items-center justify-center h-64">
        ...
        <div class="grid grid-cols-2 gap-4 mt-4">
            <div class="bg-gray-50 p-3 rounded-lg">
                <p class="text-sm font-medium text-gray-700">Male Total Time</p>
                <p class="text-xl font-bold text-blue-500" id="male-total-time">Loading...</p>
            </div>
            <div class="bg-gray-50 p-3 rounded-lg">
                <p class="text-sm font-medium text-gray-700">Female Total Time</p>
...
<script>
    ...
    async function initializeCharts() {
        ...
        // Time to Complete Module by Gender
            ...
            if (timeData.length > 0) {
                ...
                const maleTime = modulesTime.map(module => {
                    const entry = timeData.find(d => d.module_name === module && d.gender === 'Male');
                    return entry ? entry.avg_completion_time_minutes : 0;
                });
                const femaleTime = modulesTime.map(module => {
                    const entry = timeData.find(d => d.module_name === module && d.gender === 'Female');
                    return entry ? entry.avg_completion_time_minutes : 0;
                });
                ...
                new Chart(timeCtx, {
                    ...
                    options: {
                        ...
                        plugins: {
                            ...
                            title: {
                                display: true,
                                text: 'Average Time to Complete (minutes)'
                            }
                        },
                        scales: {
                            y: {
            ...
            // Update total time summary for Graph 3
                const totalTimeData = dashboardData.total_time_by_gender || [];
                const maleTotalTime = totalTimeData.find(item => item.gender === 'Male')?.total_time_minutes || 0;
                const femaleTotalTime = totalTimeData.find(item => item.gender === 'Female')?.total_time_minutes || 0;
                ...
                if (maleTotalElement) maleTotalElement.textContent = `${maleTotalTime.toFixed(1)} min total`;
                if (femaleTotalElement) femaleTotalElement.textContent = `${femaleTotalTime.toFixed(1)} min total`;
