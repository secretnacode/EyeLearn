...
<div id="module-parts-section" class="tab-content hidden">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        <?php
        // Establish a new connection for checkpoint quizzes
        $conn_checkpoint_list = new mysqli(getenv('MYSQLHOST') ?: 'tramway.proxy.rlwy.net', getenv('MYSQLUSER') ?: 'root', getenv('MYSQLPASSWORD') ?: 'niCcpkrZKKhLDhXeTbcbhIYSFJBfNibP', getenv('MYSQLDATABASE') ?: 'railway', intval(getenv('MYSQLPORT') ?: 10241));
        if ($conn_checkpoint_list->connect_error) {
            die("Connection failed: " . $conn_checkpoint_list->connect_error);
        }
            ...
            <form id="add-checkpoint-quiz-form" method="POST" action="database/add_checkpoint_quiz.php">
                ...
                <div class="mb-4">
                    ...
                    <select id="checkpoint-module-part" name="module_part_id" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-purple-500 focus:border-purple-500" required>
                        ...
                        <?php
                        $conn_checkpoint = new mysqli(getenv('MYSQLHOST') ?: 'tramway.proxy.rlwy.net', getenv('MYSQLUSER') ?: 'root', getenv('MYSQLPASSWORD') ?: 'niCcpkrZKKhLDhXeTbcbhIYSFJBfNibP', getenv('MYSQLDATABASE') ?: 'railway', intval(getenv('MYSQLPORT') ?: 10241));
                        if ($conn_checkpoint->connect_error) {
                            die("Connection failed: " . $conn_checkpoint->connect_error);
                        }
