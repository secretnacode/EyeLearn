...
<script>
    ...
    async function renderAvgScoreChart(moduleId = 'all') {
        ...
        try {
            ...
            const questionLabels = checkpointData.length
            ...
            const maleCorrectData = checkpointData.length
                ? checkpointData.map(item => item.male_correct || 0)
                : [4, 6, 3, 5];
            const femaleCorrectData = checkpointData.length
                ? checkpointData.map(item => item.female_correct || 0)
                : [5, 4, 6, 7];

            const maleWrongTotal = checkpointData.reduce((sum, item) => sum + (item.male_wrong || 0), 0);
            const femaleWrongTotal = checkpointData.reduce((sum, item) => sum + (item.female_wrong || 0), 0);
            const maleCorrectTotal = maleCorrectData.reduce((sum, value) => sum + value, 0);
            const femaleCorrectTotal = femaleCorrectData.reduce((sum, value) => sum + value, 0);

            // Update summary cards
            document.getElementById('male-checkpoint-correct').textContent = maleCorrectTotal;
            document.getElementById('male-checkpoint-wrong').textContent = maleWrongTotal;
            document.getElementById('female-checkpoint-correct').textContent = femaleCorrectTotal;
            document.getElementById('female-checkpoint-wrong').textContent = femaleWrongTotal;
