...
class CVEyeTrackingSystem {
    constructor(moduleId, sectionId = null) {
        ...
        // Python service URL - use backup version approach
    ...
    async init() {
        ...
        if (this.isConnected) {
            // Check if tracking is already active for this module (continuation from previous section)
            const existingTracking = await this.checkExistingTracking();
            
            if (existingTracking && existingTracking.is_active) {
                // Resume existing tracking session - no countdown, just continue
                console.log('🔄 Resuming existing tracking session for module:', this.moduleId, 'section:', this.sectionId);
                this.isTracking = true;
                
                // Load existing metrics from the session
                if (existingTracking.metrics) {
                    this.timers.baseFocusedTime = Math.floor(existingTracking.metrics.focused_time || 0);
                    this.timers.baseUnfocusedTime = Math.floor(existingTracking.metrics.unfocused_time || 0);
                }
                
                this.setupStatusUpdates();
                this.displayTrackingInterface();
                this.initializeTimers(); // Will continue from existing state
                console.log('✅ Eye tracking resumed - continuing from previous section');
            } else {
                // New session - show countdown
                console.log('🎬 Starting new tracking session for module:', this.moduleId);
    ...
    initializeTimers() {
        ...
        // If resuming, don't reset timers - continue from existing state
        // Only reset if this is a truly new session
        if (!this.timers.sessionStart || this.timers.sessionTime === 0) {
            this.timers.sessionStart = Date.now();
            this.timers.sessionTime = 0;
            this.timers.focusedTime = this.timers.baseFocusedTime || 0;
            this.timers.unfocusedTime = this.timers.baseUnfocusedTime || 0;
        } else {
            // Resume: continue from where we left off
            const elapsed = Math.floor((Date.now() - this.timers.sessionStart) / 1000);
            this.timers.sessionTime = elapsed;
            console.log('⏱️ Resuming timers from session time:', this.timers.sessionTime, 's');
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (data.success && data.status) {
                    // Check if tracking is active for the same module
                    const status = data.status;
                    if (status.is_tracking_enabled && 
                        status.current_module_id === this.moduleId &&
                        status.tracking_state === 'tracking') {
                        // Update section_id in the existing session
                        await this.updateTrackingSection();
                        return {
                            is_active: true,
                            status: status,
            ...
            console.log('Could not check existing tracking:', error);
            return { is_active: false };
        }
    }

    async updateTrackingSection() {
        try {
            ...
            if (response.ok) {
                const data = await response.json();
                if (data.success) {
                    console.log('✅ Switched tracking section_id to:', this.sectionId, '- tracking continues');
                }
            }
        } catch (error) {
            console.log('Could not switch section_id (non-critical):', error);
        }
    }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (moduleId) {
        ...
        // Handle page unload - don't stop tracking when navigating between sections
        // The tracking will continue in the background and resume on the next page
        window.addEventListener('beforeunload', () => {
            if (window.cvEyeTracker) {
                // Save current tracking state before navigating
                // But don't stop tracking - let it continue across page navigations
                console.log('🔄 Page unloading - tracking will continue on next page');
                // Note: Tracking continues in Python service, new page will resume it
            }
        });
    }
});
