...
if ($selected_quiz_id) {
    ...
    ");
    $completion_check->bind_param("ii", $user_id, $selected_quiz_id);
    $completion_check->execute();
    $completion_result = $completion_check->get_result();
    if ($completion_result->num_rows > 0) {
        $quiz_completed = true;
        $quiz_score = $completion_result->fetch_assoc()['score'];
    }
    ...
    if ($section_id && !in_array($section_id, $completed_sections)) {
        ...
        if ($stmt) {
            ...
            if ($stmt->execute()) {
                ...
                $response['completion'] = count($completed_sections) / $total_sections * 100;
            }
        }
    }
    ...
    if (strpos($contentType, 'application/json') !== false) {
        ...
        if (isset($data['action']) && $data['action'] === 'submit_final_quiz') {
            try {
                ...
                $stmt = $conn->prepare("
                    INSERT INTO quiz_results (user_id, module_id, quiz_id, score, completion_date) 
                    VALUES (?, ?, ?, ?, NOW())
                ");
                
                if (!$stmt) {
                    throw new Exception("Failed to prepare quiz results statement: " . $conn->error);
                }
                
                $stmt->bind_param("iiii", $user_id, $selected_module_id, $quiz_id, $score);
                $stmt->execute();
                ...
                $stmt->bind_param("iiii", $user_id, $selected_module_id, $quiz_id, $score);
                // $stmt->bind_param("iiii", $user_id, $selected_module_id, $score, $score);
    ...
    <div class="flex min-h-screen pt-16">
        ...
        <div id="sidebar" class="sidebar fixed left-0 top-16 h-full shadow-lg z-40 flex flex-col transition-all duration-300 ease-in-out">
            ...
            <div class="overflow-y-auto flex-1">
                ...
                <div class="module-item">
                    ...
                    <div class="part-item">
                        <div class="part-header p-3 bg-gray-100" data-part-id="<?php echo $part['id']; ?>">
                            <h4 class="font-medium ml-2 text-gray-700"><?php echo htmlspecialchars($part['title']); ?></h4>
                        </div>
                        
                            <!-- fix for the null chevron error -->
                    ...
                    <!-- Final Quiz Section -->
                    <?php if ($final_quiz): ?>
                    <a href="?module_id=<?php echo $selected_module_id; ?>&final_quizzes=<?php echo $final_quiz['id']; ?>" 
                ...
                <div class="quiz-container p-6 rounded-lg">
                    ...
                    <?php if ($quiz_completed): ?>
                        <div class="text-center p-8 bg-green-50 rounded-lg">
                            ...
                            <h3 class="text-xl font-bold text-green-900 mb-4">Quiz Already Completed</h3>
                            <p class="text-green-800 mb-4">Your score: <?php echo isset($quiz_score) ? $quiz_score : 0; ?>%</p>
                            <?php if (isset($quiz_score) && $quiz_score >= 70): ?>
                                <p class="text-green-800 mb-6">Congratulations! You passed this quiz.</p>
                            <?php else: ?>
                                <p class="text-orange-800 mb-6">You can retake the quiz to improve your score.</p>
                            <?php endif; ?>
                            <p class="text-green-800 mb-6">Thank you for completing this course.</p>
                            <div class="flex justify-center space-x-4">
                                <a href="Sassessment.php" class="inline-block px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
                                    View Assessment History
                                </a>
                                <a href="Sdashboard.php" class="inline-block px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700">
                                    Return to Dashboard
                                </a>
                            </div>
                        </div>
        ...
        <?php elseif ($selected_section): ?>
            <div class="max-w-4xl mx-auto">
                ...
                <div id="quiz" class="quiz-container mt-8 p-6 rounded-lg bg-white shadow-sm">
                    <h2 class="text-xl font-bold mb-6 text-gray-800">Section Quiz</h2>
                    <?php if (!empty($quiz_questions)): ?>
                    <form id="quiz-form" class="space-y-6">
                        <?php foreach ($quiz_questions as $index => $question): ?>
                        <div class="quiz-question bg-white p-4 rounded-lg border border-gray-200" data-question-id="<?php echo $question['id']; ?>">
                            <h3 class="font-medium text-lg mb-4"><?php echo ($index + 1) . '. ' . htmlspecialchars($question['question_text']); ?></h3>
                            <div class="space-y-3">
                                <?php for ($i = 1; $i <= 4; $i++): ?>
                                <div class="quiz-option flex items-center p-3 rounded-lg cursor-pointer hover:bg-blue-50">
                                    <input type="radio" 
                        ...
                        <div class="mt-6">
                            <button type="button" id="submit-quiz" class="px-6 py-3 bg-blue-600 text-white rounded-md hover:bg-blue-700">
                                Submit Quiz
                            </button>
                        </div>
                    </form>
                    <?php else: ?>
                    <p class="text-gray-600">No quiz questions available for this section.</p>
                    <?php endif; ?>
                </div>
                <?php endif; ?>
                ...
                <div class="nav-buttons">
                    ...
                    <?php if ($next_section || (!$is_section_completed && $selected_section)): ?>
                        <button type="button" 
                                class="nav-button next" 
                                id="next-section-btn"
                                data-next-url="<?php echo '?module_id='.$selected_module_id.'&section_id='.$next_section['id']; ?>"
                                data-section-id="<?php echo $selected_section_id; ?>"
                                data-has-quiz="<?php echo $selected_section['has_quiz'] ? '1' : '0'; ?>"
                            ...
                            <?php echo $is_section_completed ? 'Next: '.$next_section['title'] : 'Mark as Complete'; ?>
                            <?php echo $selected_section['has_quiz'] ? ' & View Quiz' : ''; ?>
...
(function() {
    ...
    function initializePartHeader(header) {
        // Ensure proper styling
        header.style.cursor = 'pointer';
        header.setAttribute('role', 'button');
        
        // Set up click handler that will persist
        function handleClick(e) {
            e.preventDefault();
            e.stopPropagation();
            
            const sectionsDiv = header.nextElementSibling;
            ...
            if (isExpanded) {
                // Collapse
                sectionsDiv.style.height = sectionsDiv.scrollHeight + 'px';
                sectionsDiv.style.overflow = 'hidden';
                
                requestAnimationFrame(() => {
                    sectionsDiv.style.height = '0px';
                    chevron.style.transform = 'rotate(0deg)';
                    
                    setTimeout(() => {
                        sectionsDiv.style.display = 'none';
                        sectionsDiv.style.height = '';
                        sectionsDiv.style.overflow = '';
                    }, 300);
                });
            } else {
                // Expand
                ...
                requestAnimationFrame(() => {
                    const sectionHeight = sectionsDiv.scrollHeight;
                    sectionsDiv.style.height = sectionHeight + 'px';
        ...
        // Return the handler in case we need it later
        return handleClick;
    }

    // Set up all part headers and open them by default
    document.querySelectorAll('.part-header').forEach(header => {
        console.log('Setting up part header:', header.textContent.trim());
        
        // Initialize the header
        initializePartHeader(header);
    ...
    // Open current part if there is one
    const currentPartId = <?php echo json_encode($selected_section['part_id'] ?? null); ?>;
    if (currentPartId) {
...
// Add this to fix sidebar state persistence
window.addEventListener('DOMContentLoaded', function() {
    ...
    // Auto-expand current section's part
    <?php if ($selected_section_id): ?>
    // Initialize dropdown functionality
    function togglePartSection(header, forceOpen = false) {
        const partId = header.getAttribute('data-part-id');
        const sectionsDiv = header.nextElementSibling;
        const isExpanded = header.getAttribute('aria-expanded') === 'true' || false;
        
        // If forceOpen is true, we want to open it regardless of current state
        const newExpandedState = forceOpen ? true : !isExpanded;
        
        // Update ARIA attributes
        header.setAttribute('aria-expanded', newExpandedState);
        
        // Toggle the sections visibility with smooth height animation
        if (newExpandedState) {
            // Show the sections
            sectionsDiv.style.display = 'block';
            const sectionHeight = sectionsDiv.scrollHeight;
            sectionsDiv.style.height = '0px';
            sectionsDiv.style.overflow = 'hidden';
            sectionsDiv.style.transition = 'height 0.3s ease';
            
            // Trigger reflow
            sectionsDiv.offsetHeight;
            
            sectionsDiv.style.height = sectionHeight + 'px';
            
            // Remove height and transition after animation
            setTimeout(() => {
                sectionsDiv.style.height = '';
                sectionsDiv.style.overflow = '';
                sectionsDiv.style.transition = '';
            }, 300);
        } else {
            // Hide the sections with animation
            const sectionHeight = sectionsDiv.scrollHeight;
            sectionsDiv.style.height = sectionHeight + 'px';
            sectionsDiv.style.overflow = 'hidden';
            sectionsDiv.style.transition = 'height 0.3s ease';
    ...
    // Add click event listeners to all part headers
    document.querySelectorAll('.part-header').forEach(header => {
        ...
        // Add click handler
        header.addEventListener('click', () => togglePartSection(header));
    });

    // Open the current part's dropdown
...
function togglePartSection(header, force = null) {
    ...
    console.log('Found section container:', sectionsContainer);
    ...
    const isCurrentlyVisible = getComputedStyle(sectionsContainer).display !== 'none';
    const isVisible = force !== null ? force : !isCurrentlyVisible;
...
if (document.getElementById('final-quiz-form')) {
    ...
    submitBtn.addEventListener('click', async function(e) {
        ...
        questions.forEach(question => {
            ...
            if (selected) {
                answered++;
                if (parseInt(selected.value) === correctAnswers[questionId]) {
                    correct++;
                }
            }
        ...
        try {
            ...
            const response = await fetch(window.location.href, {
                ...
                body: JSON.stringify({
                    ...
                    module_title: <?php echo json_encode($final_quizzes['title'] ?? ''); ?>
            ...
            results.innerHTML = `
                <div class="text-center p-8 bg-green-50 rounded-lg">
                    ...
                    <h3 class="text-xl font-bold text-green-900 mb-4">Congratulations!</h3>
                    <p class="text-green-800 mb-4">Your score: ${score}%</p>
                    <p class="text-green-800 mb-6">${score >= 70 ? 'You have passed this quiz!' : 'You can retake the quiz to improve your score.'}</p>
                    <div class="flex justify-center space-x-4">
                        <a href="Sassessment.php" class="inline-block px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
                            View Assessment History
                        </a>
...
// Add this code to fix final quiz access visibility
                ...
                if (data.hasFrame && data.frameData) {
                    ...
                    if (videoElement) {
                        ...
                        if (Math.random() < 0.1) { // 10% chance
                            console.log(`✅ Module part frame updated (${data.frameData.length} chars)`);
